...
/**
 * hinkey add
 * 单笔转账到支付宝账户接口 https://docs.open.alipay.com/api_28/alipay.fund.trans.toaccount.transfer/
 * @param {Object} opts
 * @param {String} [opts.out_biz_no]    商户转账唯一订单号。发起转账来源方定义的转账单据ID，用于将转账回执通知给来源方。
 * @param {String} [opts.payee_type]       1、ALIPAY_USERID：支付宝账号对应的支付宝唯一用户号。以2088开头的16位纯数字组成。 2、ALIPAY_LOGONID：支付宝登录号，支持邮箱和手机号格式。
 * @param {String} [opts.payee_account]    收款方账户。与payee_type配合使用。付款方和收款方不能是同一个账户。  abc@sina.com
 * @param {String} [opts.amount]       转账金额，单位：元。 只支持2位小数，小数点前最大支持13位，金额必须大于等于0.1元。 最大转账金额以实际签约的限额为准。
 * @param {String} [opts.payer_show_name]    付款方姓名（最长支持100个英文/50个汉字）。显示在收款方的账单详情页。如果该字段不传，则默认显示付款方的支付宝认证姓名或单位名称。  上海交通卡退款
 ...
 * @param {String} [opts.appAuthToken]  https://doc.open.alipay.com/doc2/detail.htm?treeId=216&articleId=105193&docType=1
 */
props.toAccountTransfer = function (opts) {
    var biz_content = {
...
/**
 * hinkey add
 * 查询转账订单接口 https://docs.open.alipay.com/api_28/alipay.fund.trans.order.query/
 * @param {Object} opts
 * @param {String} [opts.out_biz_no]    商户转账唯一订单号。发起转账来源方定义的转账单据ID，用于将转账回执通知给来源方。
 * @param {String} [opts.order_id]      支付宝转账单据号：和商户转账唯一订单号不能同时为空。当和商户转账唯一订单号同时提供时，将用本参数进行查询，忽略商户转账唯一订单号。
* @param {String} [opts.appAuthToken]  https://doc.open.alipay.com/doc2/detail.htm?treeId=216&articleId=105193&docType=1
 */
props.toTransferOrderQuery = function (opts) {
    var biz_content = {
        out_biz_no: opts.out_biz_no,
